...
class BookscrpperSpiderMiddleware:
...
# this will pull off the settings
...
# get the list of user agents
    ...
    # get the list of user agent
    def _get_random_user_agent(self):
        random_index = randint(0, len(self.user_agents_list) - 1)
        return self.user_agents_list[random_index]

# enable the fake user agent
...
# process the request
    def process_request(self, request, spider):        
        ...
        request.headers['User-Agent'] = random_user_agent # we re getting the rand user agent and assigning it to the header
...
# this will get a new header every time using the data it get from the fake browser endpt api
...
# kicks of the headers list which calls the api endpoint
    ...
    # process request headers (with all the fields modified)
    def process_request(self, request, spider):        
